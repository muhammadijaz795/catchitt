...
import musicTuneLight from '../profile/svg-components/music-tune-light.svg';
import locationIcon from '../profile/svg-components/location-icon.svg';
import { useEffect, useLayoutEffect, useRef, useState } from 'react';
import redHeartIcon from '../profile/svg-components/red-heart-icon.svg';
import whiteHeartIcon from '../profile/svg-components/white-filled-heart-icon.svg';
import commentIcon from '../profile/svg-components/comment-icon.svg';
import saveIcon from '../profile/svg-components/save-icon.svg';
import savedIcon from '../profile/svg-components/saved-icon.svg';
import repostIcon from '../profile/svg-components/repost-icon.svg';
import shareIcon from '../profile/svg-components/share-icon.svg';
import sendToFriendsIcon from '../profile/svg-components/send-to-friends-icon.svg';
import embedShare from '../profile/svg-components/embed-light-icon.svg';
import whatsappShare from '../profile/svg-components/whatsapp-icon.svg';
import facebookShare from '../profile/svg-components/facebook-icon.svg';
import emptyHeartIcon from '../profile/svg-components/empty-heart-icon.svg';
...
import chevronDownIcon from '../profile/svg-components/chevron-down-icon.svg';
import chevronTopIcon from '../profile/svg-components/chevron-top-icon.svg';
...
export default function PostPage() {
    ...
    const [isCommentReplyReportElipsisVisible, setCommentReplyIsReportElipsisVisible] = useState(false);
        ...
        if (isUserLoggedIn()) {
            ...
            try {
                const likeUnlikeVideo = await fetch(
                    `${API_KEY}/media-content/like/${postId}`,
                    {
                        method: 'POST',
                        headers: {
    ...
    const whatsappShareHandler = () => {
        let url = `${BASE_URL_FRONTEND}/${userName}/post/${extractVideoId(videoUrl)}`;
    ...
    const saveVideoToggler = async () => {
        if (isUserLoggedIn()) {
            ...
            try {
                const likeUnlikeVideo = await fetch(
                    `${API_KEY}/media-content/collections/${postId}`,
                    {
                        method: 'POST',
                        headers: {
                            'Content-type': 'application/json',
    ...
    const copyHandler = (msg: string) => {
        navigator.clipboard
            .writeText(`${BASE_URL_FRONTEND}/${userName}/post/${postId}`)
            .then(() => {
                showToast(msg);
    ...
    const fetchMediaById = async (videoIdPassed?: string) => {
        ...
        try {
            ...
            if (!fetchMediaResponse.ok) navigate('/');
            ...
            setTextToCopy(`${BASE_URL_FRONTEND}/${userName}/video/${postId}`);
    ...
    if (!isVideoLoaded || !info) return (
        <div className="flex flex-row items-center justify-center w-screen h-screen bg-opacity-60">
            <CircularProgress
                style={{
                    width: 40,
                    height: 40,
    ...
    return (
        <div className={style.parent}>
            ...
            <Forwardusers videoLink={postId} onOpen={sendPopup} onClose={() => setSendPopup(false)} />
            <ToastContainer />
