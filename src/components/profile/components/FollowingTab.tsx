...
function FollowingTab({onClose, following,isPublic}:any) {
        ...
        <div className={styles.div}>
            {
                following?.map((user: any) => (
                    isPublic ?   
                    <PublicUser key={user._id} user={user} onUnfollowClick={openUnfollowPopup}  popupClose={onClose}/> 
                    :
                    <User key={user._id} user={user} onUnfollowClick={openUnfollowPopup}  popupClose={onClose}/>
            ))
            }
             
            {unfollowPopup ?(
                <UnfollowPopup openUnfollowPopup={unfollowPopup} onUnfollow={onUnfollow} onCancel={onCancel} user={clickedUser} />
            ): ""}
        </div>
    );
}
