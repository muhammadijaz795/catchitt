...
const GiftRevenuePage = ({ className }: GiftRevenuePageProps) => {
    ...
    const handleFetchDiamondsData = async () => {
        try {
            ...
            console.log(`fetched diamonds::::: ${data.data.diamonds}`);
        } catch (error) {
            console.error('Error fetching profile data:', error);
        }
    };
                    ...
                    <div className={styles.middleSectionDiv}>
                        <div className={styles.settingsWrapper}>
                            ...
                            <div className={styles.suggestedContent}>
                                ...
                                <h4 className={styles.sectionTitle}>$ {revenueData.totalGiftRevenue}</h4>
                                <div className={styles.tableHeader}>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                        ...
                                        <h4 className={styles.sectionTitle}>{diamondsData.diamonds}</h4>
                                    </div>
                                </div>
                                <div style={{ width: '100%' }}>
                                    <Button sx={withdrawBtn} >
                                        Withdraw
                                    </Button>
                                </div>
                                <div style={{ display: 'flex', width: '100%', justifyContent: 'space-between' }}>
                                    <p>Daily whithdrawal limit (Remain/Total)</p>
                                    <p className={styles.limitNum}>${revenueData.userConsumption} / ${revenueData.dailyWithdrawalLimit}</p>
                                </div>
                            </div>

                        </div>
                        <div className={styles.suggestedContent}>
                            <div className={styles.settingName} style={{
                        ...
                        <div className={styles.settingsWrapper}>
                            ...
                            <div className={styles.tableHeader}>
                                <h4 className={styles.sectionTitle} style={{ textAlign: 'start' }}>{revenueData.currentMonth}</h4>
                                <h4 className={styles.sectionTitle}>in: ${revenueData.income}</h4>
                                <h4 className={styles.sectionTitle} style={{ textAlign: 'end', width: '100%', minWidth: 'max-content' }}>out: ${revenueData.outcome}</h4>
                            </div>
