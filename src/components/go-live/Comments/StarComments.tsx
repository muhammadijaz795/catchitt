...
import StarCommentFaq from "./StarCommentsFaqs";
import WhoCanSend from "./StarAllowComments";
...
const StarComment: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  const [allowStar, setAllowStar] = useState(true);
    const [showFaq, setShowFaq] = useState(false); // <-- New state
    const [showWhoCanSend, setShowWhoCanSend] = useState(false);

    if (showWhoCanSend) {
    return <WhoCanSend onBack={() => setShowWhoCanSend(false)} />;
    }
    if (showFaq) {
        return <StarCommentFaq onBack={() => setShowFaq(false)} />;
    }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <IconButton onClick={() => setShowFaq(true)} sx={{ position: "absolute", right: 0 }}>
      ...
      >
        <InfoOutlinedIcon sx={{ color: "gray", mr: 1, fontSize: 18 }} />
        <Typography variant="body2" color="text.secondary">
          You can’t pin comments when there are Star Comments in your LIVE
        </Typography>
      </Box>
      ...
      <List disablePadding>
        <ListItem>
          ...
          sx={{pr: 2}}
            primary={
              <Typography fontWeight={600}>Allow Star Comments</Typography>
            }
            secondary="Allow your viewers to send Star Comments in your LIVE"
          />
          <ListItemSecondaryAction>
            <StyledSwitch
              checked={allowStar}
              onChange={() => setAllowStar(!allowStar)}
            />
          </ListItemSecondaryAction>
        </ListItem>

        <ListItem button onClick={() => setShowWhoCanSend(true)}>
          <ListItemText primary="Who can send" />
