...
export const Home = ({ className }: HomeProps) => {
    ...
    useEffect(() => {
        if (onePost || location.pathname !== '/home') handleFetchOnePost(onePost as string);
        else dHandleFetchPosts([]);
            ...
            <div style={{ width: '100%', justifyContent: 'start' }}>
                <div className={styles.middleSectionDiv}>
                    ...
                    {paginating && postData.length > 0 && (
                        <div className={styles.paginationload}>
                            <h6>Loading...</h6>
                        </div>
                    )}
                </div>
                    ...
                    <div className={styles.postHeader}>
                        <div className={styles.tabs}>
                            {mobileTabs.map((item: string) => (
                                <p
                                    style={{
                                        color: activeMobileTab === item ? '#FFFFFF' : '#C1C6D0',
                                    }}
                                    key={item}
